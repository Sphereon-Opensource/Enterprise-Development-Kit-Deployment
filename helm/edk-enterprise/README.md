...
| `secretManagement.authority.platformProviders` | `{}` | Platform-owned Azure Key Vault or AWS KMS providers declared by the deployment, keyed by the permanent runtime provider id. Requires the distributed topology, `allowTenantManagedProviders=true`, and a `credentialSecretId` listed in `secretManagement.kubernetesMount.manifest`. The provider is created in the platform tenant at startup, is read-only through the API, and is offered to tenants according to its `sharing` block. |

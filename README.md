...
- A Sphereon license token, or access to your evaluation license issuer. The setup UI generates the deployment recipient key when it creates the license request and includes only its public key in that request. You install the issued token during platform setup, and setup must also create the first operator account. For evaluation licenses that do not chain to the embedded production root, run in `dev` mode and mount the provided root CA bundle.
